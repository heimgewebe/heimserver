...
if have ss; then
  ...
  # Uses -n for numeric ports and tightened regex to avoid false positives from peer columns (e.g. *:*)
  if ss -H -lntupn | awk '{for(i=1;i<=NF;i++) if($i ~ /^\[::\]:[0-9]+$|^:::[0-9]+$|^\*:[0-9]+$/) print $i}' | grep -q .; then
