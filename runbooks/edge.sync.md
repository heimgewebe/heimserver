...
The Edge Caddy requires the external path `/opt/weltgewebe/apps/web/build` to exist to serve the static UI locally. This is generated by the Weltgewebe stack deployment and mounted into the `edge-caddy` container. Ensure the Weltgewebe build has run or the directory exists.
